...
/*
 * Helper to get Javascript related tooling commands
 */
const getNodeTooling = service => {
  // Things we need for Node
  const tooling = {
    node: {service: service, user: 'root'},
    npm: {service: service, user: 'root'},
    yarn: {service: service, user: 'root'},
  };
...
const getServiceToolingByType = service => {
  switch (service.type) {
    ...
    case 'lagoon-node': return getNodeTooling(service.name);
    default: return {};
  }
};
...
exports.getDBUtils = service => ({
  'db-import <file>': {
    ...
    cmd: '/helpers/lagoon-sql-import.sh',
    user: 'root',
    options: {
      'host': {
        description: 'The database service to use',
  ...
  'db-export [file]': {
    ...
    cmd: '/helpers/lagoon-sql-export.sh',
    user: 'root',
    options: {
      host: {
        description: 'The database service to use',
