...
const setPrimaryServer = (services, domain, proxy) => {
  let primaryServer = '';
    ...
    } else if (!_.isEmpty(getPythonContainers(services))) {
      primaryServer = _.first(getPythonContainers(services));
      proxy[primaryServer] = [`${domain}:8800`];
    } else if (!_.isEmpty(getRubyContainers(services))) {
      primaryServer = _.first(getRubyContainers(services));
      proxy[primaryServer] = [`${domain}:3000`];
    }
